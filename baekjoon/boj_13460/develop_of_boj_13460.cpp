...
char map[10][10];
int visited[10][10][10][10], move_x[4]={1,0,-1,0}, move_y[4]={0,1,0,-1}, red_y=0, red_x=0, blue_x=0, blue_y=0, end_y=0, end_x=0, size_x=0, size_y=0;
struct Pos{
    int ry, rx, by, bx, times;
};
queue<Pos> q;
...
int use_bfs(){
    while(!q.empty()){
        Pos temp=q.front(); q.pop(); Pos next;
        ...
        for(int d=0; d<4; d++){
            ...
            if (red && !blue) return next.times+1;
