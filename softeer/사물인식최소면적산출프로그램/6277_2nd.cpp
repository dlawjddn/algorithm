...
void choose_points(int step, vector<pair<int, int> > selected, int max_y, int min_y, int max_x, int min_x){
    // 색마다 하나의 점들을 고른 상태
    if (step == color_cnt + 1){
    ...
    for(int i=0; i<colors[step].size(); i++){
        ...
        choose_points(step+1, selected, max(max_y, colors[step][i].first), min(min_y, colors[step][i].first), max(max_x, colors[step][i].second), min(min_x, colors[step][i].second));
...
int main(int argc, char** argv)
{
    ...
    choose_points(1, selected, -2000, 2000, -2000, 2000);
    cout<<answer;
}